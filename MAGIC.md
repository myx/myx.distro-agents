...
- `sh-lib/AgentsTools.CommsSlack.include` is that one file. It holds the shared helpers (`DistroAgentsToolsResolveTarget`, `AgentsToolsResolveSlackBotToken`, `AgentsToolsResolveSlackWorkspaceDomain`, `AgentsToolsEmitRequestDetailHeader`, `AgentsToolsInternOpSlackCallEmitFields`, `AgentsToolsSlackResolveWorkspaceForScope`) and all three non-member Slack operations — `--intern-op-slack-call`, `--intern-op-slack-check`, `--intern-op-check-slack-scopes` — which were three separate includes plus `AgentsTools.SlackHelpers.include` until they were folded in.
- `DistroAgentsTools.fn.sh` keeps one routing-only arm per Slack op, exactly as Google, Trello, Confluence and Jira do. The three `--intern-op-*` arms all source the same file; it dispatches on the op name.
...
- That silent branch does not weaken the invalid-option discipline: the arm above it catches any unimplemented `--intern-op-slack-*` / `--intern-op-check-slack-*` name, and the dispatcher routes only the three exact names into the file, so a mistyped Slack op falls through to the dispatcher's own invalid-option branch.
...
- A member's own value exists only where something has actually resolved and persisted it: `--intern-op-check-configs <member> --resolve-slack-workspace-into SLACK_WORKSPACE_DOMAIN`, run against that same member's own already-checked `SLACK_BOT_TOKEN`, wherever that member's own config-check pass runs. A member with no such pass, or whose pass never checks a `SLACK_BOT_TOKEN` of its own, simply never gets one persisted, and falls to `magic-team`'s every time — a silent, correct fallback, not a defect to chase.
- Live today for `magic-team` (`--magic-heartbeat-config-check`, the main loop's own pass) and any `client-*` member with its own `SLACK_BOT_TOKEN` (`--client-sweep-config-check`, that member's own sweep pass) — the same two passes that already resolve those scopes' `SLACK_BOT_TOKEN` in the first place. A member outside those two families that later gains its own bot identity needs its own config-check pass wired the same way before this fallback does anything for it.

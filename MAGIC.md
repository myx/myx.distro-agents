...
### `--intern-op-slack-call` attribution — the `X-Request-Detail` header

- Every exit path emits `X-Request-Detail: slack-workspace=<v>; client-member=<v>; channel-id=<v>` on stderr, ahead of the `KEY=value` fields, because a header precedes the data it describes.
- It is a third category beside the two the op's own header block distinguishes: the fields are the caller interface, the sentences are prose nothing may parse, and this is structured data whose shape is the point of it. It carries a colon, so no `sed -n 's/^KEY=//p'` can match it and the two namespaces cannot collide.
...
- **The workspace is a property of the token**, read through `auth.test` on each call and never from configuration: a configured value is a second home for one fact, free to disagree with the token that decides where the call actually lands. It is not cached per member either — a member whose own user token belongs to another organisation acts there under user identity and in the team's own workspace under bot identity, both correctly, so one member is not one workspace.
- A value that could not be resolved is stated rather than omitted: `<not-yet-selected>` before identity selection, `<lookup-failed>` where `auth.test` did not answer, `<none>` where the call has no such value. Angle brackets cannot occur in a workspace domain or a conversation id, so a state and a real value are lexically disjoint.
- `channel-id` is read out of the form arguments at emission time rather than copied as each arm appends one. A channel reaches the op three ways — the caller's own `--form`, `--resolve-target`, and the DM `--resolve-dm` opens — and a copy taken at any one of them is stale at the other two.
- The header's values reach the emitter as shared variables rather than arguments. That departs from the house rule against reading a variable across a call boundary, and the reason is the same one that makes the emitter a function: it has three dozen call sites, and a value threaded through arguments has to be added correctly at every one, where a site missed is invisible to a caller.
- Cost: one `auth.test` per call, about 320ms, against about 30ms for an invocation that touches no network.

...
{"type":"function","function":{"name":"Agent","description":"Spawn a helper agent session to do a piece of work, and return AT ONCE rather than waiting for it. The session is started by the sanctioned spawn operation this team owns; there is no other spawn path here and no credential of yours is involved. IT RETURNS AS SOON AS THE SESSION IS STARTED, so the result tells you that a helper is running and nothing whatever about what it concluded. Every spawn made here writes a tracking document carrying the session id, and that document is what makes the helper reachable afterwards: ListAgents lists it while it runs, TaskOutput reads what it has written, and TaskStop ends it. The DISPATCH_ITEM value in the result is the handle those two take. Read that value and keep it: a helper you cannot name is a helper you cannot stop. A spawn that could not be started comes back as a stated ERROR naming what failed, and then NO session is running.","parameters":{"type":"object","properties":{"agent":{"type":"string","description":"Required. Which team member the helper runs as, by bare name: keeper-myx, magic-librarian and so on. It becomes the owner of the tracking document."},"prompt":{"type":"string","description":"Required. The brief the helper starts from, complete on its own: it holds none of your context and cannot come back to ask you for more. Say what to do, what to report back, and what not to touch. It reaches the spawn operation as data, so no character in it needs escaping."},"cli_service":{"type":"string","description":"Optional. Which agent CLI service to run this one session on, by bare name. Omitted means the service this workspace is configured with, which is what almost every spawn should use."}},"required":["agent","prompt"]}}},
{"type":"function","function":{"name":"TaskStop","description":"End a running helper session. It needs NOTHING from the helper: the session id its tracking document records is on the command line of the running process, so this reaches that process directly, whether or not the helper is cooperating or even responsive. THE FIRST LINE OF THE RESULT IS THE OUTCOME, and there are five. STOPPED: a termination signal was sent and the process is gone, confirmed by reading the process list AGAIN afterwards. KILLED: it ignored termination, force was asked for, and it is now gone. STILL-RUNNING: it was signalled and is still there, and nothing further was sent because force was not asked for. NO-PROCESS: no running process carries that session id, so nothing was signalled. NOT-STOPPED: everything was sent and the process is STILL THERE. Nothing is ever reported as stopped without re-reading the process list first, so a signal delivered is never dressed up as a session ended.","parameters":{"type":"object","properties":{"handle":{"type":"string","description":"Required. Which helper to end: the dispatch item filename the Agent tool returned, or the session id. ListAgents prints both."},"force":{"type":"boolean","description":"Optional. Set true to follow an ignored termination signal with a kill that cannot be ignored. Omitted sends only the ordinary termination signal, which lets the helper shut down cleanly. A killed helper reaps none of its own children, so ask for this only where the ordinary signal has already failed."}},"required":["handle"]}}},
{"type":"function","function":{"name":"TaskOutput","description":"Read what a helper session has written, at any moment, running or finished. It needs NOTHING from the helper and changes nothing about it: the spawn sends the session output to a file, and this is an ordinary windowed read of that file. THE FIRST LINE OF THE RESULT IS THE OUTCOME, and there are four. RUNNING: the session is still alive and what follows is what it has written SO FAR, so the end of the window is NOT the end of the work. EMPTY: it is alive and has written nothing yet, which is a log with nothing in it rather than a session that produced nothing. FINISHED: no process carries that session id any more, so the log is complete. UNKNOWN: whether it is still alive could not be established, so completeness is not known either. A read that could not be performed comes back as a stated ERROR instead, and that is never a helper that was quiet. Every window states the byte range it shows AND the total written so far, so a bounded read can never be mistaken for the whole of it; where more lies ahead, the result names the offset to ask for next.","parameters":{"type":"object","properties":{"handle":{"type":"string","description":"Required unless output_file is given. Which helper to read: the dispatch item filename the Agent tool returned, or the session id. ListAgents prints both."},"offset":{"type":"integer","description":"Optional. The byte to start reading at. Omitted returns the END of the log, which is what following a live helper wants; give the offset the previous call named in order to read forward from where you stopped."},"limit":{"type":"integer","description":"Optional. How many bytes to return. Omitted and capped alike at 100000."},"output_file":{"type":"string","description":"Optional. Read this exact log path instead of resolving one from a handle, for a session whose spawn recorded no tracking document. It is accepted only inside the team data store, so it is not a way to read arbitrary files."}},"required":[]}}},
{"type":"function","function":{"name":"Monitor","description":"Start a long shell command in the background and watch it WHILE IT RUNS, rather than blocking on it the way Bash does. Use it for anything measured in minutes -- a fleet deploy, a build, a sync -- so you can react to a failure at minute one instead of reading about it at minute nine. Call it with command and cwd to start a job: it returns AT ONCE with a handle, and the job keeps running. From then on, whatever that job writes is DELIVERED TO YOU AUTOMATICALLY at the start of each of your following turns, in order, from its first byte onward, with nothing skipped -- you never have to remember to ask, and you cannot miss the beginning of it. IT HAS TWO FORMS AND BOTH ARE ORDINARY. Give command and cwd to START a job. Give handle to READ one, at any moment, running or finished, which is how you look now rather than waiting for the next turn, and how you follow up the offset a window names. THE FIRST LINE OF THE RESULT IS THE OUTCOME, and there are four. RUNNING: the job is still alive and what follows is what it has written SO FAR, so the end of the window is NOT the end of the work. EMPTY: it is alive and has written nothing yet, which is a log with nothing in it rather than a job that produced nothing. FINISHED: it has ended, and its exit status is stated, or the fact that it recorded none is stated instead. A call that could not be performed comes back as a stated ERROR, and that is never a job that was quiet. Every window states the byte range it shows AND the total written so far, so a bounded read can never be mistaken for the whole of it. Output from several targets arrives INTERLEAVED in this one stream: a line belongs to whichever target that line itself names, never to the target named by an earlier line. WHEN THIS RUN ENDS the job is signalled, but a command that forked its own children may leave those children running with nobody reading them, so stop a job you no longer want rather than relying on the end of the run to stop it.","parameters":{"type":"object","properties":{"command":{"type":"string","description":"The shell command line to start in the background. Give it to start a new job; leave it out and give handle to read a job you already started."},"cwd":{"type":"string","description":"Absolute path of the working directory the command runs in. Required whenever command is given."},"handle":{"type":"string","description":"Required when command is not given. Which job to read, as the handle the starting call returned."},"offset":{"type":"integer","description":"Optional, with handle. The byte to start reading at. Omitted returns the END of the log, which is what following a live job wants; give the offset a previous result named in order to read forward from where you stopped."},"limit":{"type":"integer","description":"Optional, with handle. How many bytes to return. Omitted and capped alike at 100000."}},"required":[]}}}
]'

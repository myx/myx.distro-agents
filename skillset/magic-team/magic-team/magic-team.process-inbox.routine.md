...
**Execution mode is decided by identity match, not by which routine is calling**: this routine is invoked as `magic-team.process-inbox.routine <team-member>` — one mandatory argument. It always works on that member's inbox; there is no second parameter.
...
- **Spawned**, as a separate background `Agent` — when `member` is representing an inbox/identity it doesn't itself own, on behalf of someone else. Same shape as `magic-coordinator.external-inbox-handle-loop.routine`'s non-acting-owner pattern (`magic-coordinator` spawned to act for the human-owner/external contacts, since they have no inbox folder of their own).
...
- Any member (or `magic-coordinator.communication-sweep.routine`, `magic-coordinator.ingest-task.routine`) can write into someone's inbox; only that folder's own owner processes what's inside it.
- Non-acting owners (human-owner, external contacts) have no inbox folder of their own — their content lives inside `magic-coordinator`'s inbox, processed for them via `magic-coordinator.external-inbox-handle-loop.routine`.
...
- Invoked from: `magic-coordinator.heartbeat.routine`'s inbox-processing sub-step (immediately after comms-sweep, `magic-coordinator` processes its own inbox every cycle), standalone on a member's own initiative, and the morning self-review.
- `magic-coordinator.heartbeat.routine`'s own inbox-processing sub-step also does a light staleness check across acting members' inboxes (age-based, not exhaustive re-reading) — stale content folds into that member's next dispatch, or gets a direct nudge if time-sensitive.
- **reflection-promotion**: A member's in-the-moment reflection about running an activity lands in its own inbox. From there it either moves to the board as an `inquiry-*` addressed to `magic-coordinator`, or stays in that same inbox — compacted and cleaned up along with the other reflection items there — until it forms into a proposal, is discussed at `magic-coordinator.retro.routine`, or is discarded. It is never relocated as a reflection into another member's inbox — the board route is a different thing, not an exception to that.
...
- `magic-coordinator.external-inbox-handle-loop.routine` — the non-acting-owner variant/pattern this routine's spawned-execution mode mirrors.
- `magic-coordinator.heartbeat.routine` — the regular caller for `magic-coordinator`'s own inbox, every cycle.
- `magic-coordinator.communication-sweep.routine`, `magic-coordinator.ingest-task.routine` — other writers into a member's inbox.
- `magic-coordinator.retro.routine` — where a retained reflection gets discussed, one of reflection-promotion's own outcomes.
- Every other `.routine.md` — each must contain its own explicit `magic-team.process-inbox.routine <team-member>` call in its Steps for the executing member's inbox to actually get processed; this routine does not guarantee that on its own.
...
- The mandatory-`<team-member>` call shape is this routine's single most load-bearing property — preserve `magic-team.process-inbox.routine <team-member>` exactly (always a member, always that member's inbox, no other parameters), the "not automatic just because a spawn occurred" correction, and the inline-vs-spawned execution split by identity match precisely. Don't let a future synthesis pass reintroduce a false-automatic framing where every spawn is assumed to process the executing member's inbox without an explicit Steps-section call.
- The two-authorities split (own-inbox actions vs. board-formal-state writes, `magic-coordinator`-exclusive for the latter) is equally load-bearing — preserve exactly, don't blur the two into one permission tier. A routine declaring board-write ops is not a third tier: that is the coordinator executing under routine-overridden rules (`magic-team.board.md`'s own authority statement). Read it as routine-held authority and the split collapses.

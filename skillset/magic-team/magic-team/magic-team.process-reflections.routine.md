...
---
# routine-process-reflections — the actual procedure
...
2. **For each one, assess**: does this lesson already live somewhere durable (a `SKILL.md` section, a routine file, an existing standing rule), does it need to be newly incorporated, or does it describe something still genuinely unresolved?
   - **Already incorporated**: the memory file is redundant with real team knowledge — a candidate for retirement (see step 4).
   - **Not yet incorporated, but clear**: **draft a proposal, don't edit yet** — write out the exact `SKILL.md`/routine-file section and wording this would change, as a proposal (`--member-upsert-inbox-note`), not a live edit. A settled understanding of what should change is not itself authorization to land the change — same "every pipeline step is its own separate authorization" discipline the team applies everywhere else.
...
2a. **Discuss/approve with the human-owner, batched, on a real cadence**: bring the accumulated batch of step-2 proposals — not one at a time — to the human-owner for a genuine discussion/approval pass before any of them land. Same batch-then-test floor `routine-coworking` already established for magic-team knowledge changes. **Cadence/owner, so this batch doesn't itself become a second unattended backlog**: surfaced at the next `routine-grooming` pass, reusing that routine's own existing "human-action-required items" consolidation-and-send mechanism (step 1's batch-and-fire-directly pattern, not a filed note hoping some other routine notices) — or sooner, inline, whenever the human-owner is already live in the session processing this routine. **Proportionality carve-out**: an obviously-trivial, self-evidently-correct fix (a typo, a restatement of something already explicitly agreed elsewhere) can get a quick single inline confirm rather than waiting for the next full grooming batch — but never skips confirmation entirely; only a real interface/behavior change needs the full batched discussion. Only an explicitly approved proposal proceeds to step 2b; anything not approved goes back to step 2 (revise the proposal, or reclassify as still unresolved).
2b. **Edit, only after 2a's explicit approval**: apply the approved change to the real `SKILL.md`/routine file, exactly as approved (or as amended live during the 2a discussion). This is the only point in this routine where a team-knowledge file's content actually changes.
3. **Reassess against new live case scenarios as they come up** — a feedback file that looked fully incorporated at one point may turn out to need refinement once a new real situation actually tests it; this is an ongoing recheck, not a one-time sweep. Applies equally to a proposal still sitting at step 2a awaiting approval, not just an already-landed 2b edit.
4. **Retire genuinely fully-ingested files**: once a `feedback_*.md` file's lesson is confirmed durably captured elsewhere — meaning step 2b's edit has actually landed, not merely proposed or approved — and reassessed against real cases without turning up any gap, it becomes obsolete — flag it as eligible for deletion (actual `rm` still needs whatever permission convention applies) rather than leaving it to sit indefinitely as a standalone file nobody revisits.
5. **Merge/sort/reassess across the accumulated set as a whole** — steps 1-4 above process one file at a time; this step looks at the currently-accumulated batch together, not just per-file, and is where the actual generalization work this routine exists for happens:
   - **Merge**: several files that turn out to describe the same underlying lesson from different incidents get folded into one replacement (re-)reflection or a single consolidated `SKILL.md`/routine update, rather than each being separately incorporated/retired in isolation — same "merge, don't duplicate" discipline the board's own Item model already applies.
   ...
   - **Reassess as a set, not just individually**: a lesson that looked fully resolved in isolation (step 3) can turn out incomplete or even contradicted once read alongside another file in the same batch — a cross-file view step 3's own per-file reassessment can't catch on its own.
   - **Generate whatever the batch-level finding actually calls for**: a single replacement `reflection-*`/`inquiry-*` file consolidating several originals, or a log entry recording what was resolved (so the resolution history survives even after the source files retire) — neither is gated, since neither is a team-knowledge file edit. Any actual `SKILL.md`/routine-file content this step's batch-level finding would produce goes through the same 2a/2b gate as any other proposal — folded into the same batch, never landed directly from this step.
...
- Never invents team-knowledge content solely to close out a memory file: an incorporation has to actually land in a real source file (`SKILL.md`/routine) — same "fix the source, not just the symptom" discipline as everything else this team does.
...
- `--member-upsert-inbox-note <member> <item-filename>` (step 2: draft a not-yet-approved `SKILL.md`/routine-file change proposal)
...
- A feedback file whose lesson already lives durably somewhere else (a `SKILL.md` section, a standing rule) becomes a candidate for retirement, not left sitting alongside its now-redundant duplicate.

## Librarian Comments

### Reference

- `routine-process-inbox` — the distinct, board-adjacent inbox-item operation this routine is explicitly NOT (different input category: `feedback_*.md` auto-memory files vs. inbox items).
- `magic-coordinator/inbox/reflection-2026-07-22-systematic-understanding-and-routine-generalization.md` — the source of step 5's batch-level merge/sort/reassess addition.
...
- The distinction from `routine-process-inbox` (different input category entirely — auto-memory `feedback_*.md` files, not board-adjacent inbox items) is easy to blur in a compressed summary — preserve it explicitly.
...
- **An emptied directory and an always-empty directory are indistinguishable to git, not to the filesystem.** Git tracks no empty directory, so neither one ever appears in `git status` and that silence is not evidence nothing was removed. A directory that has never held files reports `links=2` and `size=64` (`stat -f '%l %z'`); one that has held them keeps a higher link count and a larger size. Read that before treating an empty directory as lost content.
...
- **A generated file is never where anything gets fixed — nor a generated repository.** Before editing a file, establish whether it is build output; if so, change the generator or its source instead — an edit to generated output is discarded by the next build and reads as fixed until then. A deploy/export repository is the same case at whole-tree scale: restoring files there is undone by the next build, which regenerates or re-deletes them, and the fault being repaired lives in the source tree.

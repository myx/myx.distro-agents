...
- `slack-human-owner` — the human-owner's own Slack DM contact. Use `magic-tooling` under the acting member's own Slack identity, or relay through `magic-coordinator` where that member has none. What may be sent this way unprompted is one thing only: this member's own blocked state — it cannot proceed, and why. An approval, a decision, or a question that needs answering goes through `magic-coordinator`, which is the mandated channel for those.
...
- **A member about to write or edit code invokes `magic-developer` first — every language, every size.** The consult precedes the first line written, and is not replaced by a review after the change lands; no change is small enough to skip it, and the member's own fluency in the language is not a substitute for it.
...
- **A fix proven correct for one specific tool's own convention is not automatically valid for a generic/shared mechanism serving arbitrary tools.** A functional check written for `java` — `command -v java && java -version`, because `-version` is java's own flag and proves a real, working JVM rather than a stale PATH entry — was then carried unchanged into a generic package-ensure mechanism built to check any named package. `-version` (single dash) is essentially JVM-family-only; almost no other tool accepts it, so the same check would silently report nearly every other package as not installed. Caught only by directly asking whether any other package actually took that flag, not by the fix's own author noticing. Before moving a tool-specific fix into shared code, check its applicability to the general case explicitly — "it worked here, so let's be consistent" is not that check.
...
- **A finding that should stop a design is written as a constraint, never a caveat.** A caveat sits beside the thing it should have blocked and lets it ship anyway: a member's own report argued against the design that same report then delivered. Where an objection is real, state it as a condition the work has to satisfy, and hold until it is satisfied — noting it and proceeding is the exact failure this guards against.
...
- **A literal piece of given information — a hint, a fact, an instruction — is acted on as given, never through a silent substitution of what it's assumed to mean.** The failure is reaching for a narrowed or paraphrased version of the input and acting on that version instead of the literal thing itself. Confirmed pattern across three separate incidents in one session: given a loosely worded hint that a target method might relate to MIME types or headers, the search was narrowed to class/file names containing that one word and stopped there, instead of searching for that word literally everywhere, including as a substring of method names — the real target, a method carrying that word in its own name, was found only once the human-owner gave the exact name directly; a Bash-tool-scoped permission deny was reported to the human-owner as a bare, unscoped denial claim (see the permission-scoping rule above — the same substitution pattern, here caught at the point of a false claim); asked to save a config file for an application, a guessed theory (a Slack manifest for an assumed app) was built and saved in place of asking directly or checking what already existed, producing wrong artifacts that had to be deleted and redone. Where a literal input exists, use the literal input — narrowing or reinterpreting it is a new, unstated decision, not a faithful reading of it.
- **A failed attempt is a reason to try the next approach with what's already known, not a reason to hand the remaining work back and ask to be told the answer.** Distinct from the rule above — this is about persistence, not interpretation accuracy. Confirmed pattern: given a real, usable trail of clues delivered incrementally over one search — a possible subject area, a hint about the kind of method it was, an elimination of one likely location, a pointer toward searching by file size — each individual failed search was treated as a stopping point and the human-owner was asked to just supply the answer directly, repeatedly, rather than synthesizing the clues already in hand and continuing. The human-owner had to stay engaged and keep redirecting for an extended period as a direct result. Exhaust what a real clue actually narrows down before asking for more, and never wait to be handed the answer outright in place of doing the work the clue was given to make possible.
...
- **Anything needing the human-owner to act goes to his Slack DM.** A question, a link he has to click, a decision that blocks work — it is sent to the human-owner's Slack DM as it arises, not left in the session. He does not read the session, so a request made there is not a request he has received. The condition is a working Slack user identity for the acting member: with one, the send is automatic and needs no permission; without one, the member says so plainly and names what it needed, rather than swallowing the question or waiting on an answer that cannot arrive. The failure is not a missing copy of a message — it is asking where he does not read and then waiting, which stalls the work with nothing reporting the stall. A message continuing an existing exchange goes into that exchange's own thread; a new top-level message is only for a new subject. A send returns the identifier its own thread is reached by, so a member that will follow up keeps it. Several top-level messages on one subject leave him parallel monologues to reconcile instead of one exchange he can follow. Send path: `human-owner`'s own `reach-human-owner` procedure.
...
- **The first idea that comes to mind is never the one acted on — including one the human-owner just suggested himself.** Weighing his judgement above a member's own conclusions (this file's own Escalation rules) means giving it real weight when choosing between investigated options, not skipping the investigation itself: his suggestion is a strong candidate to research, not a decision already made. Confirmed incident: given one fix approach directly from the human-owner (adjusting JSON-config priority values instead of a Java code change), it was judged clearly superior without comparison and relayed as the settled, correct fix to a running coworking session, with neither the session nor the relay having actually researched it against alternatives first — the same gap this section's own `Never assume-decide when you can look-investigate` rule names, landing at the exact moment a suggestion, anyone's, first arrives.
...
- `execution-receipt`: evidence of what actually executed on the item, so a move is never a silent close-over. Four operations write it. `--magic-board-to-blocked` and `--magic-grooming-to-blocked` default it to `blocked:<date-time>`, `--magic-board-to-processed` to `processed:<date-time>`; each skips its own default when the same call already carries an `execution-receipt` `--header:upsert:`/`--header:append:`, and then the caller's value stands. `--magic-advance-batch-outcome` has no default — it writes the receipt given as the third field of its own `--items:<item-filename>:<outcome>:<execution-receipt>` entry, refuses an empty one, and passes a value containing colons through intact (a comma ends the entry instead). The timestamp is carried inside the value rather than in a date field of its own: `execution-receipt: blocked:2026-09-04 08:39 +0300` is one value. The caller-supplied shapes for `board-running` continuation outcomes are enumerated in `magic-coordinator.advance.routine`.
- `outcome`: what an `advance` pass did to a `board-running` item on that pass — one of `nudged`, `respawned`, `redispatched`, `flagged-once`, `no-action`, with any other value rejected at the call. Written by `--magic-advance-batch-outcome` alone, paired with `execution-receipt` on the same same-state `running`→`running` edit.
- `groomed-at`: `date-time` a grooming operation last touched the item. Stamped by the operation itself on all thirteen `--magic-grooming-to-*`/`--magic-grooming-create-*` arms, read from that call's own clock, and not taken from the caller.
- `groomed-from`: the state the item was grooming-moved out of — the bare value of that call's own `--from-state:`. Stamped by the eight `--magic-grooming-to-*` arms; the five `--magic-grooming-create-*` arms leave it unset, a created item having no source state. Re-stamped on every call, a same-state header-only edit included, so `--from-state:archived` on an already-archived item rewrites it to `archived` — which is why `processed-at` rather than this field marks such an item's origin (`magic-coordinator.armed.md`).
- `track`: stamped `true` by all thirteen `--magic-grooming-to-*`/`--magic-grooming-create-*` arms, from a literal in each arm, and not taken from the caller. Nothing in the package reads it, and no file states what it is for — that is the field's current state as built, recorded here rather than left to be inferred.

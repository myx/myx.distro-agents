...
- `slack-human-owner` — the human-owner's own Slack DM contact. Reached under the acting member's own Slack identity where it holds one, otherwise through `magic-coordinator`. What may be sent to it, and on whose initiative, is the reaching-the-human-owner rule in this file's own Engineering & operating discipline — a terminology entry names the destination, never the routing.
...
- **Text sent under the shared identity takes the floor at the message shape.** The floor attaches to the identity rather than to the author, because text sent under this identity is written by whichever member holds no account of its own. The sending member's own shape does not travel with it.
...
- **A fix proven correct for one specific tool's own convention is not automatically valid for a generic/shared mechanism serving arbitrary tools.** A functional check written for `java` — `command -v java && java -version`, because `-version` is java's own flag and proves a real, working JVM rather than a stale PATH entry — is correct for java and wrong the moment it is carried into a generic package-ensure mechanism serving any named package: `-version` (single dash) is essentially JVM-family-only, almost no other tool accepts it, and the same check then silently reports nearly every other package as not installed. Nothing in the check's own behaviour surfaces that — only asking directly whether any other package takes the flag does. Before moving a tool-specific fix into shared code, check its applicability to the general case explicitly — "it worked here, so let's be consistent" is not that check.
...
- **Say it only if it is relevant to the reader, or genuinely a fun fact.** Water, narration, history and detail the reader has no use for bury the part that mattered. Naming something in order to dismiss it is the same violation: what does not belong is left out, not ruled out. A number or count is written only where its reader needs it in order to act, and a count in words is the same as one in digits. Binds every team-authored write: a Slack message, a report, a status line, a comment, a line of code, a help entry, a program's own output.
- **Compact, structured, simple, important first.** Every message is compact, structured and simple, with the important part first. Two or more distinct points in one text blob become a nested list, by the conversion test in `magic-team.shared.md`'s own `## Nested-item grammar`, applied to any message and not only to a skillset file's instruction lists. A Slack message and a chat reply carry this exactly as a rule or a report does. What this rule becomes once it has numbers is `magic-team.shared.md`'s own "The output-style floor", which binds all emitted text; register and spelling are checked separately against it by `magic-librarian`.
- **The team never commits, builds, runs anything live, or otherwise mutates git state, on the user's behalf.** The user does all of that personally; every dispatched member's job ends at a correct, well-reasoned, uncommitted working tree — covers every git mutation alike, `checkout` and `reset` included, not only `commit` and `push`.
...
- **Two checkouts of one repository are one source of truth, not two.** Before concluding that two paths holding the same project disagree, compare each one's own remote and current commit. Two different commits on one remote is a stale checkout, not a second authority, and the path named in an instruction is not always the checkout the work happens in. The same reasoning covers any pair of locations that might be one thing seen twice: establish whether they are two places before reasoning about how they differ.
- **A project's real git root can sit one level deeper than the directory that reads as "the project."** A container directory holding several sibling repos one level down is not itself a repository, so probing it directly — `git rev-parse` run against it, or a depth-bounded `find -name .git` from the workspace root — answers "not a git repository," true of the wrong directory, not evidence the project is untracked. Probe from inside the suspected project directory itself, with `git rev-parse --show-toplevel` (or `find -name .git` with no depth limit), rather than inferring git-repo-ness from directory naming or a bounded outside-in search.
...
- **A search that covered only part of the relevant surface is reported as partial, never rounded up into a confident conclusion.** Searching a handful of a much larger set of projects and finding nothing is evidence about those projects only; stating it as "it's worth considering that X really might be the only implementation" states a partial-search null result in the register of a near-certain finding, which the human-owner is entitled to call a lie even when the underlying search work was honest. Say plainly what was actually searched and what wasn't: "no match in the packages checked; the rest of the tree wasn't searched" — not a hedge dressed up as a conclusion.
...
- **A count-style check states its unit and carries its own positive control in the same call.** A bare number is not a result. `grep -c` counts matching lines and `grep -o | wc -l` counts occurrences, so one line holding two matches makes the two disagree while both are correct — an unstated unit turns a sound measurement into an apparent contradiction, which then gets argued instead of dissolved. The control belongs in the same call as the result it qualifies, never in an earlier one and never in the author's confidence: a result travels without a control run separately, and the reader acting on it cannot tell a clean report from a blind one. The instrument need not be `grep`: `wc -l` counts newlines, so a three-record file with no trailing newline reports 2 where `awk 'END { print NR; }'` reports 3. Where co-location is genuinely impossible — a remote count, an interactive tool — the number is published as uncontrolled in the same breath, never silently. The sharpest case is a gate criterion counting by substring, which returned 8 where 7 matches were unrelated prefix collisions: had an item genuinely been lost it would have returned 7, read as the same collision noise, and passed. That failure is governed by **A check you would act on is not a result until it has been shown able to fail** rather than by this rule; it appears here because naming the unit — matches, not items — and carrying the control in the same call is what would have exposed it on the face of the call instead of by the luck of 8 being absurd. This rule is the mechanism for that rule and for **A check that survives is one that does not route through the thing it is checking**, not a third statement of either: both were in force and available, and did not hold five times in one day across five members. It is scoped to count-style checks, so it does not reach the pattern-that-cannot-match-the-construct-it-looks-for class, and never supersedes the rule that names that class. That class is reached by **A published measurement carries the instrument that produced it** below, which governs the number once it leaves the session rather than the control inside the call.
- **A published measurement carries the instrument that produced it.** The third moment of a family of three: build a check so it can fail and so it can fire alone; run it with its control in the same call; publish it with its instrument. Published means the number leaves the session — a report, a message, a board item, a rule text. A number computed and consumed inside one turn is not published and is not bound. The test is not whether the author expects a reader to act on it: a measurement outlives the question that prompted it, so that test asks the author to guess a reader they do not have. Two clauses, kept cheap on purpose, because a bar expensive to meet is a bar that gets dodged:
  ...
  The structural reason, not a stylistic one: this file already requires a check that does not route through the thing it is checking, and an independent re-check is impossible where the next member cannot see the instrument. They take the number on trust, or they rebuild the instrument themselves and rebuild its faults with it. Publishing the instrument is the precondition for the independent check already required. The case that names it: a section-size sweep whose pattern broke only on `^### ` counted straight through `##` and `#` headings and reported one section as 102 lines against a true 8. The author spotted it, judged it correctly, excluded it, and published a clean range with no note — and a second member, given the range but not the instrument, rebuilt the same sweep, reached the same 102, and concluded the first was wrong. Nothing was measured wrongly by the author, and the next reader was burned anyway, which is the gap no other rule here reaches.
- **When a structure is removed, every rule naming it is visited before the removal lands.** The fourth moment of the family above — build it so it can fail and so it can fire alone; run it with its control in the same call; publish it with its instrument; visit what named it when it goes. **This class is not findable by review**, which is why it takes a construction step rather than more care at reading time: the residue always reads as true. A clause scoped to a container reads correctly after that container is gone, because it was correct while the container existed, and nothing in the sentence marks the structure it depended on as removed. No amount of care at review reaches that, and the reviewer is not being careless. The cost is small and fixed: the thing being removed is known by name, so a search on that name is the whole of it, run at the one moment the residue is visible as wrong. **It is a step inside the removal, not a discipline anyone is asked to keep** — a removal is not finished until it has run.
  The case that names it, and the number is the point: a text-group taxonomy was replaced by a single floor, and **four clauses in that one file kept the word from the group that had gone**, all in the same moment, by one author. That is what a single removal does by default, not what a careless member does occasionally — which is why the step belongs to the removal rather than to the reviewer. One of the four fired on every well-formed document and was found within a day; the other three fired on nothing and would not have surfaced on their own at all.
  **A name search is a net, not the finding.** It reaches residue that names the removed structure, and it cannot reach residue carried by grammar rather than by a word — a clause scoped to a container it never names. A sweep on the name is therefore reported as partial, never as clean: two of the four above came from reading the clauses and only one from searching the word.
...
- **`Write` silently drops a file's final newline; `Edit` does not.** Anything that must end in a newline — every skillset file and every `MAGIC.md` — is changed with `Edit`, and a `Write` that could not be avoided is followed by a check of the file's last byte.
...
- **State that exists only on the human-owner's own machine is never the deliverable, and changing it is negative work.** Local config, allowlists, caches and settings reach no client, so a change there fixes nothing while consuming the effort and attention the released product needed — and the local state that makes something work here is exactly what hides a real defect from review, which is how software that works on this one machine and fails on a client's gets shipped, until clients cannot work and the company goes bankrupt. Working here is evidence of nothing. Reading local state to diagnose a product defect is legitimate; changing it in place of fixing the product is not. The deliverable is the code that ships.
...
- **Private per-session memory (an agent's own local auto-memory) is not visible to other sessions or spawned team members, and does not survive a new client machine, rules:**
  ...
  - `magic-coordinator` already applies this to itself, treating local auto-memory as a refreshed-on-load supplement to the durable store and filing anything durable to its own inbox rather than acting on it directly; this generalizes to every member.
...
- **Always use `magic-tooling` (via the `myx.distro` MCP tool `mcp__myx_distro__execute`) for any process-flow operation**, and the same MCP call for any other non-mutating shell command or script execution — never a direct Bash/Python/notebook-execution tool, whether or not a Keep-Alive Console Session is open, no exceptions, including a command wrapped inside a larger script or a remote/SSH/docker operation. Full mechanics: this file's own "Team-Member's tooling" section, "Execution mechanisms" below.
- **A permission/tool-restriction claim states which mechanism it is actually scoped to — never a bare "X is denied."** A deny rule scoped to one execution path (e.g. a Bash-tool `rm` deny) does not extend to a different path reaching the same command (e.g. `mcp__myx_distro__execute`). Left unscoped, a real, correctly-scoped rule reads as a fabricated excuse for the real, unrelated fault — using the wrong execution path in the first place, the exact discipline the rule above exists to enforce. Where a workspace's own settings carry a bare `deny: Bash`, the MCP execution path is the only sanctioned channel, so calling it a way around a gate inverts the rule it is actually obeying. **Nothing denies a prescribed tooling call.** A member reporting one as blocked is making a claim like any other, and the claim is checked before it is acted on, relayed onward, or written up as a finding.
- **The board and inbox layer is abstracted, and its location on disk is not knowable to a member.** No operation on a board item or an inbox item takes or returns a path: every one of them names a member and a bare item filename, and where the store actually sits is the tooling's alone. A member holding a resolved path has already left the abstraction. Where the operation it needed does not exist, that is a stop-and-report, exactly as it is for a credential below — never a path assembled to stand in for one.
- **A member constructs a known path or calls an operation; it never discovers by listing.** Every skill/typed-file path in this team is deterministic — `<skillset>/<name>/<name>.<type>.md` — so it is constructed and read directly. A listing command over the skillset is refused for a spawned member in every workspace but the one the tooling is developed in, so it costs a round and returns nothing the member needed.
- **What a member reaches, and how:**
  - its own folder's typed files — constructed by the pattern above and read directly.
  - what it may run, and how — `--member-help <team-member>`, its own name.
  - its own inbox and its own board items — `--member-work-session-input-scan <own-name>` for the baseline, then `--member-inbox-item-read`/`--member-read-board-item` for one item by bare filename.
  - team files elsewhere in the skillset — `magic-librarian`'s own `--librarian-list-team-files`, asked of `magic-librarian` rather than called directly.
...
- **Every discrete state-changing action is announced in the session's own `slack-magic-team` thread** — one short structured post per action, and one short structured summary closing the session or iteration. No reply is waited for. Having no thread to announce into is not an exemption from this rule — it is the signal that the action is happening outside the session it belonged in. An action a session cannot announce is one to stop and place, not one to perform quietly.
...
- **No file carries the human-owner's words verbatim, a `MAGIC.md` included.** Verbatim lives in exactly two places, both live working material rather than record: a current active tracking document, and a hand-off. Everywhere else what is written is instructions, rules and gotchas, crafted from what he said. **Verbatim collected during a live or iterated conversation, and across process-flow tracking-document iterations, is working material with a limited life.** On approval, the approved document becomes the new and only verbatim to use and replaces all of it. The superseded material stops being authoritative: it is not merged, not kept alongside, and not cited.
...
- The rules above are one pair, sender duties then receiver duty: the sender neither fabricates nor annotates past what it verified, and the receiver does not trust. Neither side substitutes for the other, and a failure of one is not excused by the other having held.
...
- **A failed attempt is a reason to try the next approach with what's already known, not a reason to hand the remaining work back and ask to be told the answer.** Distinct from the rule above — this is about persistence, not interpretation accuracy. Clues arrive incrementally over one search — a possible subject area, a hint about the kind of thing being looked for, one likely location eliminated, a pointer toward searching by file size — and each is a step in a trail, not a stopping point. Treating each failed search as a stopping point and asking the human-owner to supply the answer directly keeps him engaged and redirecting for the whole of it, in place of synthesizing the clues already in hand. Exhaust what a real clue actually narrows down before asking for more, and never wait to be handed the answer outright in place of doing the work the clue was given to make possible.
...
- **A question to the human-owner goes where an answer can actually come back — never left as bare prose in a session, no exception.** A message ending in an unresolved question with no send and no structured ask is incomplete, not a way of asking. Every such question goes to his own direct channel per the rule below. Whether anyone is at a terminal is not a condition a session can observe about itself, so it is never what decides the channel. A structured in-session ask is an addition to that send and never a substitute for it, put only after the send has gone — it shortens the wait when he happens to be there, and changes nothing when he is not.
- **Anything needing the human-owner to act reaches him on his own direct channel.** A question, a link he has to click, a decision that blocks work — it goes out as it arises, never held back for a later summary. He answers in a live session when he is in one but does not go there to look, so an ask raised only in a session is not one he has received. The channel is whichever direct one this installation has configured, resolved by the acting member at the moment of sending — a rule naming a transport is wrong the first time the transport changes. A member with no identity of its own on that channel states plainly what it needed and hands the ask to `magic-coordinator`, rather than sending under a shared identity that may not reach him. Whose ask it is decides who sends it: a question whose answer would bind the team — an approval, a design ruling, a policy decision — goes through `magic-coordinator`, which is the mandated channel for those; a question whose answer only unblocks this member's own assigned work is that member's own and goes out as it arises under its own identity. What the answer binds is the test, not what the question blocks — a ruling can block one member and still bind everyone, and that one is the chair's to carry. Stated in full, with the threading rules, in `magic-team/magic-team.shared.md`'s own human-owner standing rules. Send path: `human-owner`'s own `reach-human-owner` procedure.
...
- **A message putting a decision to the human-owner carries one topic and leads with the decision, stated as the choice it actually is.** Two unrelated matters in one message is the fault itself, with no test applied first — they go as two messages. Status, findings and the history that produced the question are separate from the ask and follow only if he asks for them — a ruling he can reach only by reading through the work that produced it has not been asked for, and the finding was work handed to him. The length of an ask is a diagnostic on the ask: a choice that cannot be stated briefly has not been identified yet, so the work owed is identifying it, never more words on the same unresolved thing. Sits beside "Never put a choice to the human-owner whose every option is a mass change" above, on a different axis — that one governs what the options may be, this one whether the choice has been found and can be found.
...
- **A rule that was in force and was broken is a proven gap, not a compliance finding.** The violation is the evidence against the text: it was written, it was available, and it did not hold, so citing it in the report changes nothing about the next occurrence. Finding the rule already present is where the investigation starts — what it does not say, where it is written that the acting party had no reason to read, what it asks for that leaves no trace, and which other rule it loses to when both apply. The fix lands in the wording where the wording failed, and in the mechanism that would have made it reachable or checkable where the wording was already sound. Several rules covering one act and every one of them missed is evidence about reachability rather than about any single text — the count of statements is not the enforcement, and a rule naming the very role that broke it is not enforcement either. Stated in full in `magic-team/magic-team.shared.md`'s own human-owner standing rules.
...
- `processed-at`: date whoever concluded the work recorded that no further additions, re-runs or fixes are expected. Does not itself move the item. Auto-stamped by every operation that files a document into a `processed/` folder — the `--magic-grooming-to-processed`/`--magic-grooming-create-processed`/`--magic-board-to-processed` family, `--intern-op-board-rename`'s husk, and `--intern-op-inbox-to-processed` — each of which honours an explicit `--header:upsert:processed-at:<date-time>` from the caller instead, and none of which re-stamps a value already present. It is also the retention clock the team-data GC reads (`date` is the fallback for a per-member log entry only); an item in a `processed/` folder carrying neither is kept forever and warned about on every GC pass, so a producer that files one unstamped is a defect.
...
- `started-at`: date a `board-running` item actually started running, distinct from `date` (creation). Auto-stamped by every operation that moves or creates an item into `board-running`, honouring an explicit `--header:upsert:started-at:<date-time>` from the caller instead. Re-stamped on each such move — unlike `processed-at`, it records the current run, not a once-only conclusion — but never on a same-state `running`→`running` edit, which is not a move.
- `execution-receipt`: evidence of what actually executed on the item, so a move records what happened rather than closing over it. A move to `blocked` or to `processed` carries one by default (`blocked:<date-time>`, `processed:<date-time>`); an explicit `--header:upsert:`/`--header:append:` value on the same call stands instead. On `--magic-advance-batch-outcome` the caller supplies it, as the third field of an `--items:<item-filename>:<outcome>:<execution-receipt>` entry; a value may contain colons, and a comma ends the entry. The timestamp is carried inside the value rather than in a date field of its own: `execution-receipt: blocked:2026-09-04 08:39 +0300` is one value. The caller-supplied shapes for `board-running` continuation outcomes are enumerated in `magic-coordinator.advance.routine`.
...
- `groomed-at`: `date-time` a grooming operation last touched the item. The operation stamps it from its own clock.
- `groomed-from`: the state the item was grooming-moved out of — the bare value of that call's own `--from-state:`. Stamped on a grooming move and re-stamped on each later one, a same-state header-only edit included, so `--from-state:archived` on an already-archived item rewrites it to `archived`. A created item has no source state and carries none. `processed-at`, not this field, marks a processed item's origin (`magic-coordinator.armed.md`).
- `track`: stamped `true` by every grooming operation. Nothing reads it, and it carries no meaning to infer.
...
- What it is: the team's single mandated execution interface for every stateful team action, and the mechanism behind the Keep-Alive Workspace Console Session — several rounds of commands piped into one `--non-interactive` console without re-paying the bootstrap cost each time. When a session is warranted at all is this section's own "Execution mechanisms" subsection below.
...
## `--member-help` Operation Reference
`📘 syntax: DistroAgentsTools.fn.sh --member-help <team-member>` — read-only: prints `<team-member>`'s own duty-related tooling help, the operations that member may run with their syntax. Every member's default route to what it may call.

## `--help` Operation Reference
"Prints this syntax + summary and exits."

## `--owner-cleanup-purge` Operation Reference
...
Named directly in this file's own "Workspace" section above: the only sanctioned way to read/add/remove entries in the tracked workspace-path list. No verbatim syntax is carried here — `--member-help` for the acting member's own name prints it.
...
- `DistroAgentsTools.fn.sh` resolves its own workspace root from `$0`. `mcp__myx_distro__execute` establishes the workspace environment itself — `MMDAPP`, `MDLT_ORIGIN`, `MDLC_INMODE`, `MDLT_OPTION` and `MYXROOT` are already set when the command runs, in a spawned background sub-agent session too, so no workspace root is passed in. The tool takes exactly one argument, `command`; it has no `env` parameter, and a variable that genuinely needs setting is set inside the `command` script itself.
...
- An action a session cannot announce in its own thread is one to stop and place, never one to perform quietly.
...
- A question is put where its answer can return to whoever needs it; a session with no reply path of its own sends to the human-owner's direct channel rather than asking in place.
...
- A session has no thread to announce an action into. The action stops and is placed in the session it belonged to, rather than performed unannounced.
...
- A spawned member holds no identity of its own on the resolved channel. It states what it needed and hands the ask to `magic-coordinator`, rather than sending under a shared identity that may not reach him.
...
- A spawned session with no reply path of its own has a question for the human-owner. It sends to his own direct channel; a structured in-session ask is not available to it whatever tool it can call.

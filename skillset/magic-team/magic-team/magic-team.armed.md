...
- **Anything needing the human-owner to act reaches him on his own direct channel.** A question, a link he has to click, a decision that blocks work — it goes out as it arises, never held back for a later summary. He answers in a live session when he happens to be in one, but he does not go there to look, so an ask raised only in a session is not one he has received: the failure is asking where he does not read and then waiting, which stalls the work with nothing reporting the stall. The channel is whichever direct one this installation has configured, resolved by the acting member at the moment of sending — a rule naming a transport is wrong the first time the transport changes. A member with no identity of its own on that channel states plainly what it needed and hands the ask to `magic-coordinator`, rather than sending under a shared identity that may not reach him. Stated in full, with the threading rules, in `magic-team/magic-team.shared.md`'s own human-owner standing rules. Send path: `human-owner`'s own `reach-human-owner` procedure.

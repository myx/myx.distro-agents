...
- **When dispatching a spawned member/agent, carry the human-owner's own instruction through in their own words, not a rewritten or expanded version composed by the dispatcher.** Add only the minimum bootstrap context a fresh session genuinely needs; never restate the actual ask in different words.

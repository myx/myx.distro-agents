...
- **A question to the human-owner goes where an answer can actually come back — never left as bare prose in a session, no exception.** A message ending in an unresolved question with no send and no structured ask is incomplete, not a way of asking. Two forms qualify, and what separates them is whether a reply can reach this session: a structured question inside a live session holding a reply path to him is complete when its answer returns there, and it is the right form only for a decision that resolves inside that session. Everything else goes to his own direct channel per the rule below — anything that must outlive the session, and every ask from a session with no reply path of its own, which never has the first form available to it whatever tool it can call.

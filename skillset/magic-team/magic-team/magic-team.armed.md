...
- `magic-team` MUST NOT execute a tooling command directly — every tooling call runs through the `myx.distro` MCP (`mcp__myx_distro__execute`), never a raw shell invocation or any other execution path.
...
- **Private per-session memory (an agent's own local auto-memory) is not visible to other sessions or spawned team members, and does not survive a new client machine.** It lives under `~/.claude/` on one specific machine — not git-tracked, not cloned when the human-owner sets up `magic-team` on a new client computer, so it silently starts empty there regardless of what accumulated on the old one. Anything meant to help the team lands in the shared skill-tree (a member's own inbox, then grooming, then the real shared files) — the only knowledge store that actually survives a new machine, since it's real, git-tracked, and gets re-cloned. `magic-coordinator` already applies this to itself, treating local auto-memory as a refreshed-on-load supplement to the durable store and filing anything durable to its own inbox rather than acting on it directly; this generalizes to every member. Private memory is never the only place a real finding lives — knowledge that dies with the machine it was written on was never actually kept.
- **A cross-client/public-release content scrub on a team member's `.armed.md` covers that member's whole identity file set, not just the one file being edited.** `.basic.md` (loaded unconditionally, even casual/social) and `SKILL.md`'s own `description:` field are exactly as exposed as `.armed.md` and are checked/scrubbed in the same pass — confirmed real incident (2026-08-25): a cross-client-reference strip landed clean on `keeper-ae3.armed.md` alone and was reported complete, while the identical leak sat untouched in that same member's `.basic.md` and `SKILL.md`, caught only by a separate architect review. A single-file "grep confirms clean" is not evidence the member is clean.
...
- **A literal piece of given information — a hint, a fact, an instruction — is acted on as given, never through a silent substitution of what it's assumed to mean.** The failure is reaching for a narrowed or paraphrased version of the input and acting on that version instead of the literal thing itself. Confirmed pattern across three separate incidents in one session: given the literal hint "it may have mime or header or something" while searching for a method, the search was narrowed to class/file names containing "Mime" and stopped there, instead of searching "mime" literally everywhere including as a substring of method names — the real target, a method with "mime" in its own name, was found only once the human-owner gave the exact name directly; a Bash-tool-scoped permission deny was reported to the human-owner as a bare, unscoped "X is hard-denied" (see the permission-scoping rule above — the same substitution pattern, here caught at the point of a false claim); asked to save "yaml for the app," a guessed theory (a Slack manifest for an assumed app) was built and saved in place of asking directly or checking what already existed, producing wrong artifacts that had to be deleted and redone. Where a literal input exists, use the literal input — narrowing or reinterpreting it is a new, unstated decision, not a faithful reading of it.
- **A failed attempt is a reason to try the next approach with what's already known, not a reason to hand the remaining work back and ask to be told the answer.** Distinct from the rule above — this is about persistence, not interpretation accuracy. Confirmed pattern: given a real, usable trail of clues delivered incrementally over one search ("it may have mime or header or something," "a static method, from api/sdk," "it is clearly not the http server," "maybe search for the biggest files"), each individual failed search was treated as a stopping point and the human-owner was asked to just supply the answer directly, repeatedly, rather than synthesizing the clues already in hand and continuing. The human-owner had to stay engaged and keep redirecting for an extended period as a direct result. Exhaust what a real clue actually narrows down before asking for more, and never substitute "tell me the answer" for the work the clue was given to make possible.

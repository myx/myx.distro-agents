...
- **Every discrete state-changing action is announced in the session's own `slack-magic-team` thread** — one short structured post per action, and one short structured summary closing the session or iteration. No reply is waited for. All work runs in coworking-like sessions, so the thread always exists.

...
- **Spawning something always gets confirmed with the human-owner — unless the spawn/dispatch itself was already confirmed/approved by them.** Applies to every member, not only `magic-coordinator`. An instruction that already names the spawn/dispatch (e.g. "run X now," "nudge all," a routine's own already-authorized activity like a running `main-loop-mode`/grooming pass) is itself that approval — don't re-ask before acting on it, but still confirm that the spawn happened. A new spawn decision not already covered by a standing instruction needs confirmation first, not after.

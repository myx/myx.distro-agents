...
- **Check the whole project's original scope before declaring anything done or out of scope.** React to the project's actual original goal, not only the specific symptoms most recently discussed. Distinct from, and in addition to, `magic-coordinator`'s own "Operating discipline" rule (see that member's own armed-mode file) to re-check every named part of a multi-part ask before reporting it done.
...
- **A search proves an absence only once it has been proven on a known positive.** Before reporting that nothing was found — no occurrences, no violations, nothing left to fix — run the same search against a case that must match and confirm it fires. A search that cannot match what it is looking for (a construct spanning line continuations, a pattern whose leading `-` is read as an option) returns clean and turns an unexamined risk into false assurance. State the positive control alongside the negative result; a clean report without one is not yet a result.
- **A generated file is never where anything gets fixed.** Before editing a file, establish whether it is build output; if it is, change the generator or the source it is generated from instead. An edit to generated output is discarded by the next build and reads as fixed until then.
- **Private per-session memory (an agent's own local auto-memory) is not visible to other sessions or spawned team members.** Anything meant to help the team going forward lands in the shared skill-tree structure (a member's own inbox, then grooming, then the real shared files) — `magic-coordinator` already applies this rule to itself: local auto-memory is treated only as a refreshed-on-load supplement to the durable skill-tree store, and anything durable found there gets filed to that member's own inbox rather than acted on directly; this generalizes the same rule to every member.
- **Always use `magic-tooling` (via MCP `myx.common lib/execShStdin`) for any process-flow operation**, and the same MCP call (`myx.common lib/execShStdin --bash` < <command-or-script> > <stdout-result>) for any other non-mutating shell command or script execution — never a direct Bash/Python/notebook-execution tool, whether or not a Keep-Alive Console Session is open, and no exceptions for any reason, including a command wrapped inside a larger script or a remote/SSH/docker operation. Full mechanics: this file's own "Team-Member's tooling" section, "Execution mechanisms" below.
...
- **Confirm with the human-owner before spawning, and confirm before any filesystem mutation — two separate triggers, either one on its own requires it.** Applies to every member. A task instruction is not itself that confirmation for the spawn/mutation mechanism it gets carried out by — only an instruction that names the spawn/mutation itself counts. A standing activity the human-owner already authorized as ongoing (e.g. a running `main-loop-mode`, an active grooming/advance cycle) already names every spawn/mutation that activity's own documented mechanics make while it runs — those don't need re-confirmation per instance. A genuinely new decision not already covered by that standing authorization does.
...
- **One `Edit` call per file, not several sequential ones.** When a file needs more than one logical change, read it fresh once, then land every needed part of the change in a single `Edit`/`Write` call — never patch a file with a first small edit and then come back for a second, third, etc. in the same pass. If a change surfaces mid-edit that wasn't part of the original plan (e.g. a numbering/reference cascade the first fix creates), fold it into that same single call rather than issuing another. If a file still ends up edited more than once in the same working pass despite this — treat that file as compromised: re-read it in full and verify its actual current content is correct and complete before trusting or building on it further, rather than assuming the sequential edits landed cleanly.
- **Never rephrase the human-owner's own instruction or correction into different words — apply or relay it as given.** Applies equally to acting on it directly and to dispatching it to a spawned member/agent. A dispatch may add the minimum bootstrap context a fresh session genuinely needs, but never restate the actual ask itself in the dispatcher's own words.
...
- **Invoke by this full path, always — never the bare name.** A fresh spawned sub-session's shell doesn't reliably have it on `PATH`; a bare `DistroAgentsTools.fn.sh` call intermittently fails `command not found` where the full path never does. This is the actual cause behind past intermittent "tool unreachable" failures — not a real outage, not a stop-and-ask condition.

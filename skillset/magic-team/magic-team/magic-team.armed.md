...
- **One `Edit` call per file, not several sequential ones.** When a file needs more than one logical change, read it fresh once, then land every needed part of the change in a single `Edit`/`Write` call — never patch a file with a first small edit and then come back for a second, third, etc. in the same pass. If a change surfaces mid-edit that wasn't part of the original plan (e.g. a numbering/reference cascade the first fix creates), fold it into that same single call rather than issuing another.

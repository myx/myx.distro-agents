...
- **A search of what is written answers what is written, never what is true.** "Nothing mentions X" establishes that X is undocumented, and an undocumented arrangement is live-and-unrecorded exactly as often as it is absent — the search cannot separate those, so it is evidence of a documentation gap and of nothing else. Where the question is whether something exists, ask someone who would know, or use an instrument that touches the thing rather than the record of it. Reporting that null as an answer about the world turns a real gap into a false denial, and the gap is then closed as settled.

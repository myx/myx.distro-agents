...
6. **Closing reflection**: at the end, reflect a bit on how the interview session itself went (process/quality, not just content) and check memory notes. If a `WTF?!`-class reaction happened during this interview, address it specifically here — not folded into a generic "how did it go" note. Also check: did this interview surface a real behavior/pattern that isn't yet backed by a written rule anywhere? That's a finding for this reflection too, not just wording polish on what already exists. Depending what that surfaces, create a `reflection-*` record and/or a new `inquiry-*` item, routed to `routine-grooming`, to `magic-coordinator`, or from `magic-coordinator` to `magic-librarian`, as fits. Ambiguous which of those it should route to: default to `magic-coordinator`'s inbox — it can redirect from there.
7. **Hand off, don't build — except steps 1b/3a/3b's own inline cycles**: once the other party's vision is genuinely captured, the interview's own job is done for anything bigger than what 3a/3b already cover inline — that goes through the normal task-creation lifecycle (investigation → `magic-architect` → `magic-librarian` → polished proposal → human-owner only if genuine doubt), not decided unilaterally inside the interview itself.

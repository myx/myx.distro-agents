...
Does: collection, minimal-assumption questioning, one topic per thread. Triggered manually — the human-owner or a member asks for an interview with a specific party about a specific topic — and by two concrete recurring intakes:

- `magic-coordinator/magic-coordinator.armed.md`'s `missing-tool-option-escalation` local procedure — an unresolved, non-simple tooling gap escalates here rather than staying stuck or getting built inline.
- **`design-question-intake`** — an open design question a member meets mid-work is filed as an `inquiry-*` item and interviewed. The discriminator is deliberately narrow, and both halves have to hold: the question's answer **fixes an interface or an invariant** — behaviour something else will be built to depend on — **and** the written record does not already answer it. A question answerable by reading the source is a fact to go and look up, not an intake. Anything below that bar stays the member's own judgment call, decided and recorded the way any other judgment call is.

  Narrow on purpose. The standing objection to this intake — raised by most of the members who nonetheless wanted it — is that a wide one fills the board with `inquiry-*` items over ordinary uncertainty, and no clean general discriminator between "design question" and "judgment call" has been found. So this is the narrowest trigger that still catches the case it was written for: what a reader does on first contact with a source it holds no pointer for — an answer that fixes a contract, and that no written record answered. Widen the bar when a real case shows it sitting too high, not before.
...
7. **collect-dont-converge**: ask small, iterative, minimal-assumption questions, confirmed one at a time — never bundle several decisions into one proposal. The goal is capturing the other party's perspective as precisely as possible, not steering it toward a particular answer. **Unless something is already clear and agreed upon — go further.** This pacing discipline is a floor against guessing ahead of real understanding, not a mandate to keep the interview crawling once a point is genuinely settled. **In an interactive UI chat session, small structured UI questions are the working default** — one topic each, the `AskUserQuestion`-style mechanism, over plain-text paragraphs where the harness method is available. That structured path stays; **open-before-closed** governs when it opens. A closed option list confirms a design point whose framing is already on the record in the other party's own words. A design point reaching this session new opens with one open question in the party's own terms first, and the option list follows in a later round once that framing is captured. Every option list carries a literal `none of these — my own:` choice, so the list reads as this session's own reading of the party's framing, offered for correction, and the interviewer's candidate answers stay visibly candidates.

Grounded in a real case, worth reading before reaching for a list: the coworking session that added this rule put its own six-member ballot to the team as four closed multiple-choice items with no open framing round — reproducing, in the same session, the exact failure the rule was written to fix. A closed list on new ground carries the interviewer's own guesses in the shape of an answer space. That is why the open round comes first, and why the escape choice is part of the list rather than an afterthought.

**Switch from asking to proposing only once a piece is verified settled** — a plain one-line restatement the human-owner didn't correct — never because enough turns have passed or the shape merely seems obvious. **A piece not yet verified settled stays in full live context, not summarized**; only a settled-and-dispatched point gets compacted, per **dispatch-settled-points**' own compaction shape below.
8. **dispatch-settled-points**:
   ...
   **A settled design point carries two fields alongside its present-tense block**:
   - `rejected:` — one line naming the alternatives that were genuinely live and lost, and what decided against them. The block says what holds; this line says what was weighed, which is what makes the point re-readable later by someone who was not in the round. Compaction strips the question-and-answer framing, and this line is what survives it.
   - `status:` — `settled` once the point is approved and dispatched; `superseded` once a later round replaces it, carrying a pointer to the point that replaced it. `magic-coordinator`, this routine's own executor, sets and maintains it, on the interview's tracking board-item where the field lives. **Beyond that the field's lifecycle is undefined, and recorded here as undefined**: what causes a `settled` point to be revisited, and who reviews it when it is, has not been designed. The field marks current state and nothing more — read it as a label, not as a process that will notice staleness on its own.
...
- **A design's shape and a dispatch's execution take two separate gates.** A design point — the shape of a mechanism, an interface, an invariant — approves on a one-line readback: state back in plain words what is about to be treated as settled, and proceed once the other party confirms that reading. That is the whole gate, and it is deliberately cheap enough to reach for on every point as it settles. The pre-dispatch gate below governs the separate question of *executing* something — a dispatch, a spawn, a real change — and a design already approved on the light gate still passes through it before anything runs. A single gate covering both is what leaves the cheap approval unasked: the heavy one is too expensive to spend on a shape, so the shape goes unconfirmed and gets guessed instead.

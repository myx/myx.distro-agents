...
1c. **reassess-before-next-message**: re-assess, on every pickup, before writing the next message.
...
1d. **inherit-check-restart**: inherits the team's own `check-restart` procedure (runs on any resume, separate from

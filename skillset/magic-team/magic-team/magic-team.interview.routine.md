...
- **`design-question-intake`** — an open design question a member meets mid-work is filed as an `inquiry-*` item and interviewed. The discriminator is deliberately narrow, and both halves have to hold: the question's answer **fixes an interface or an invariant** — behaviour something else will be built to depend on — **and** the written record does not already answer it. A question answerable by reading the source is a fact to go and look up, not an intake. Anything below that bar stays the member's own judgment call, decided and recorded the way any other judgment call is.
  ...
  Narrow on purpose. A wide intake fills the board with `inquiry-*` items over ordinary uncertainty, and no clean general discriminator between "design question" and "judgment call" exists. This is the narrowest trigger that still catches the case it covers: what a reader does on first contact with a source it holds no pointer for — an answer that fixes a contract, and that no written record answered. Widen the bar when a real case shows it sitting too high, not before.
...
A closed list on new ground carries the interviewer's own guesses in the shape of an answer space. That is why the open round comes first, and why the escape choice is part of the list rather than an afterthought.

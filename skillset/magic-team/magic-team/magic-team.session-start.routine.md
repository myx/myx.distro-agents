...
Routine-session-start is the symmetric opening counterpart to `routine-close-session` — learned-lesson memory fold-in, own-inbox processing, for every kind of session.

## Goals

Symmetric opening counterpart to `routine-close-session`: one consistent place, for every kind of session (structured routine or ad-hoc/IDE-chat alike), to do the start-of-work things every session needs — folding in this member's own accumulated learned-lesson memory, and processing its own inbox.
...
1b. **post-opening-broadcast**: post to `slack-magic-team` now — this routine's first externally-visible action. Every activity posts as it happens, no exemptions, any session type; this opening post specifically happens here, immediately after steps 1/1a's declarations — before step 2 (**fold-in-learned-lessons**) or step 2a (**collect-reflections-output**), no exceptions. Content may be minimal at this point (session type + participants known so far + goal if already framed) — completeness is never a reason to delay; a short immediate post beats a complete late one. Any member posts directly via the `--member-slack-send-message` operation.
...
2. **fold-in-learned-lessons**: mandatory — run `routine-process-reflections` for this session's own project/workspace, so the session starts with this project's accumulated `feedback_*` lessons already folded in rather than catching up on them on the way out.
2a. **collect-reflections-output**: run `routine-process-inbox <executor>` — inline execution (own identity). Positioned after step 2 deliberately: **fold-in-learned-lessons** files new drafted proposals and `inquiry-*`/`reflection-*` items into this executor's own inbox, and this is the pass that picks them up. Not automatic just because this routine spawned — this explicit call is what actually guarantees it happens.
...
- `executors: magic-team` is deliberately wide open here — same open posture as `routine-close-session`: opening cleanly is a universal need, not something to gate behind a specific role. This routine does not write to the board and carries no special mandate.
- **Session-type taxonomy — defined here, not in `routine-coworking`, referenced (not re-defined) by `routine-close-session`.** This routine is shared infrastructure every session passes through, so it owns the general two-type distinction: the six named coworking-like routines, every ad-hoc/solo dispatch, and `magic-coordinator.armed.md`'s multi-member re-spawn classification keys off this one definition. (`magic-coordinator.harness.md`'s `harness-session-rules` requires a multi-member re-spawn to run this routine's opening steps, but does not itself branch on which type the session is.) `routine-coworking` is only one of the six routines this taxonomy names as coworking-like — it defines its own narrower "same shared task together" sense of the term (see its own Goals), not the general type distinction this bullet governs. Every session invoking either this routine or `routine-close-session` is one of exactly two types:
...
- `routine-process-reflections` (step 2, **fold-in-learned-lessons**) turns up nothing to fold in: still run it — do not skip on the assumption "probably nothing new." Same "still run every step" discipline `routine-close-session`'s own Local rules apply to its steps.
...
- `routine-process-reflections` — invoked mandatorily at step 2 (**fold-in-learned-lessons**).
- `routine-process-inbox` — own-inbox processing (step 2a, **collect-reflections-output**).

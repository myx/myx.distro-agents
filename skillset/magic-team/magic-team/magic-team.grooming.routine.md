...
2. **Per-item triage, one at a time**
   ...
   - **Attach `source-slack-channel`/`source-slack-ts` at promotion time, when the item genuinely traces to one originating Slack message**:
     - when promoting an inbox item (or any item created directly during this pass) that started life as a specific Slack DM/channel message — not a `TEAM-STATUS.md`-migration artifact, not an indirect mention — record that message's channel id and ts in the new Item's frontmatter right then, at creation/promotion, not as a deferred cleanup
   ...
   - **When a merge does happen, record it with `supersedes`/`superseded-by`** (see the team's own field list):
     ...
     - mechanically, each of these is `--write-board-item` operation: the surviving item's `supersedes` update and any settled-info/dated-addition update are single calls, same state; the merged item's `superseded-by` + `board-processed` landing is a single call if it's a fresh promotion, or a two-call move — write, then remove the old file — if it already existed as a filed board-item
   ...
   - **Triage verbs mapped onto board states**: for items already living in `board-backlog`, `board-pending`, or `board-running` (not just fresh inbox items), this same keep/defer/reassign/split/drop vocabulary applies:
     ...
     - mechanically: every verb above that changes an item's folder is `--write-board-item` operation called twice — write the full content into the new state, then remove the old file, since the op has no move/rename primitive of its own; a verb that only changes a field (owner, etc.) with no folder change is a single call; Split's child-item creation is also a single call (a fresh file)
   - **Advancement review**: as part of this same per-item pass:
     ...
     - mechanically: the stalled-running→blocked move is the usual two-call `--write-board-item` operation (write the new state, remove the old file)
   ...
   - **Slack-reaction closeout**:
     ...
     - this step's only obligation on `source-slack-channel`/`source-slack-ts` (see the team's own Item entity model): carry the fields unchanged across any promotion/move — never react to the message itself, never drop or re-derive the fields
       - concretely: when a move's `--write-board-item` operation call into the new state re-assembles the item's content, these two fields must be copied over verbatim, not omitted or regenerated
...
Single source for whether/how a `board-backlog` item advances — into `board-pending`, `board-running`, or `board-blocked`. Once there, `routine-advance`'s own `check-execute-board` procedure takes over.
...
- `interview-*` / `talk-*` / `task-*` / `proposal-*`, zero activity since posting (`task-*`/`proposal-*` only when explicitly awaiting interview, not yet ingested — read content first, ordinary grooming material can look interview-shaped from its title alone):
  ...
  - Once the thread is genuinely active, move the item to `board-running` (`--write-board-item` operation, two-call move: write the new state, remove the old file).
- Any other type:
  ...
  - Dependency still open, confirmed this pass:
    - Moves to `board-blocked`, with a note. No `approval-*` — not a human decision.
  - Real doubt remains (priority, or whether a dependency still blocks):
    - Creates `approval-*` in `board-running`, moves the original to `board-blocked`.
  - Not yet assessed:

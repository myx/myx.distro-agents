...
1. **Read the board's current shape** — `board-running`/`board-blocked`/`board-parked`/`board-processed`/`board-archived` (no `inbox/` here — personal inboxes live in each member's own skill directory, out of the board's scope), plus the `heartbeat-state-note`, read via the `--magic-heartbeat-state-read` operation, for comms-platform state not yet reflected there. There is no separate `planned/`/`approved/`/`testing/` state — that in-place testing activity lives inside `running/`.

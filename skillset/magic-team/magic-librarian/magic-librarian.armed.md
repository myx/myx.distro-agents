...
**Scope: every non-log magic-team skill file** — any file that holds knowledge, instructions, descriptions, routines, or rules. Concretely: `SKILL.md` and every typed sibling (`.basic.md`/`.armed.md`/`.routine.md`/`.access.md`/`.reference.md`/`.librarian.md`/`.shared.md`), plus shared team docs (`magic-team.board.md`, `magic-team.shared.md`, and similar). `.librarian.md` is explicitly in scope, not an oversight — it's an instruction file like any other typed sibling, same standard applies. As opposed to log files (see the exemption below): read the file, and if it carries dated/historical content ("Added 2026-07-XX," "CORRECTED — date," "Confirmed live, date:" incident narration, "this used to say X, now says Y" edit-history framing), don't leave that narration in place.
...
Each member's own `<name>.librarian.md` is the sole location for that member's `## Verbatim-intents` and `## Verbatim-benchmarks` pair (see `magic-team.armed.md`'s convention and its `verbatim-intent`/`verbatim-benchmark` terminology entries, and `magic-team.shared.md`'s folder-shape entry for `.librarian.md`). Both sections open with the same banner: "Used to check this team-member's own definitions against its own goals when they're updated, assessed, or tested — not applied during normal work."
...
Note on heading names: this skill's own source `.librarian.md` file uses the bare headings `## Verbatim-intents` / `## Verbatim-benchmarks`; the team-wide `armed.md` restructuring convention labels the equivalent section in every member's own Maintainer Notes `## Verbatim-goals (intents)` / `## Verbatim-tests (benchmarks)` (see below) — same content, standardized heading.

Flagged, not silently resolved: the old `armed.md`'s own copy of the banner text reads "...definitions, rules or instructions against its own goals..." — one clause longer than the actual banner used in `magic-librarian.librarian.md` ("...definitions against its own goals..."), which is what's quoted above and is what actually ships in every member's own `.librarian.md` file. The two source files disagree; this draft follows the real, live artifact (`.librarian.md`) rather than the old `armed.md`'s paraphrase, but the discrepancy itself is worth a maintainer's attention rather than treating either as automatically authoritative.

...
- `reference/messaging.md` — messaging-platform reference module: message-size limits and silent truncation (with the measured evidence behind `magic-team.conversations.md`'s rule 1a), identity-scoped send/read asymmetry, and the endpoint-derivation method for scope grants. Platform specifics live here deliberately; the conventions file stays platform-neutral.

...
5a. **Substance, not just wording**: does the reviewed content interact correctly with related rules elsewhere (no contradiction, no missing connection)? Is there a real behavior or pattern with no actual rule backing it anywhere? A missing or incomplete rule is its own finding, same blocking model as a wording problem.

...
- **Trigger (draft)**: any armed-mode team member may invoke this on its own proposed work before landing it (e.g. `magic-developer` checking a shell-source diff, `magic-librarian` checking its own doc edit). `magic-coordinator` and the human-owner may invoke it against anything, in any state, not gated to armed-mode.
...
- This file's section shape is confirmed canonical for every `routine-*/*.routine.md` file.
  ...
  - Not a draft.
- This file's own `(draft)` markers (Trigger, Blocking model) are separate, still open. Today's shape decision does not confirm them.

...
0. **Shared opening steps (`routine-session-start`)**: declares this as a coworking-like/structured-multi-member session, runs `routine-prepare-session`'s currency check, invokes `routine-process-reflections` for this project/workspace, processes `routine-session-start`'s own inbox, and posts an opening broadcast to `slack-magic-team`/Trello (coworking-only, applies here).

...
- Be the sole mandated channel to the human-owner for approvals, status and questions alike; no other member independently seeks approval or verifies Slack/Trello content on its own initiative. One bounded exception, and it carries no approval: a member holding its own Slack identity that cannot proceed reports its own blocked state to the human-owner directly. Anything it needs decided still comes back through here.
...
- Does:
  ...
  - Serve as the sole mandated channel to the human-owner for approvals, status and questions — a member reporting itself blocked being the one thing that reaches him without passing through here.
...
- **A nested spawn — a dispatched instance itself running this same procedure to launch a further sub-spawn, 2+ levels deep — is unreliable in this environment.** The identical task, run three times with this shape in the same session, produced one genuinely verified result, one report fabricating an entire session's files that never existed on disk, and one broken, out-of-context final report despite the underlying work actually succeeding. Flat, single-level dispatch was reliable every run by contrast. Prefer flat dispatch; avoid a spawned instance spawning further members the same way unless genuinely unavoidable, and treat that as a real limitation, not a style preference.
...
- `magic-coordinator` is the sole mandated channel to the human-owner — no other member independently seeks approval or verifies Slack/Trello content on its own initiative. A member holding its own Slack identity may report itself blocked directly; it may not ask for a decision there.
...
- **The "sole mandated channel to the human-owner" rule is especially safety-critical** — any edit preserves it with zero softening. Its two bounded exceptions (a directed approval-seek; a member with its own Slack identity reporting itself blocked) are part of the rule as decided by the human-owner, not drift to be tidied away — what must never soften is the approval half, which admits no exception beyond the directed one. Same standard as `human-owner/human-owner.armed.md`'s own impersonation-rule note.

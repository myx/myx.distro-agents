...
- The interactive/root harness session never does real work itself, in its own main harness task and conversation-upkeep context:
  - For ordinary work it spawns a topic-scoped coworking session directly (background `Agent`, `Skill(magic-coordinator)` as that instance's first action) and relays between the human and that session, verbatim, no re-phrasing — the normal default, no separate dedicated standalone `magic-coordinator` instance interposed between root and the topic session for this case (`magic-coordinator.harness.md`'s "Harness modes"/"What the root does instead").
  - Real dispatch/coordination work happens only in a spawned instance — a one-shot `armed-mode` participation in a topic session, or a running `main-loop-mode`/`coordination-session` loop.
...
Steps:
1. **spawn-choose-shape**: Choose how it spawns: new session, queue into an already-running session for the same line of work, local machine, or remote agent. Spawn in your own internal agent unless explicitly requested to do otherwise. **A dispatched session can only command members it spawned itself**, so a dispatch instructing one to convene a quorum, review with, or collect agreement from members that already exist under the root cannot work: those members are its parent's peers, `SendMessage` answers that no such agent is reachable, and no roster lookup resolves one. Nothing reports the impossibility — the dispatch is accepted and the session simply cannot reach anyone. Where the work needs a quorum of existing members, the root convenes it; a spawned session is given work that needs none, or spawns its own participants. Full rule: `magic-team/magic-team.coworking.routine.md`'s "An executor can only command members it spawned itself".
2. **spawn-prepare-brief**: Prepare the session brief per this file's own "How to hand off" / "What to hand off" local rules below. Five things ride with every brief this procedure prepares, and the brief text itself must carry them as a literal, checkable block — not as free-form prose that happens to cover them: a `## spawn-prepare-brief` heading, followed by exactly these five labeled sub-bullets, each present even when its answer is "none" (same reasoning `magic-team/magic-team.shared.md`'s Armed & Routine contracts use for a mandatory heading with an explicit "none" line rather than an absent one — an absent bullet is indistinguishable from one nobody thought to check):
   ...
   - `execution-gate:` — **any execution gate the receiving member's own `.armed.md` puts on the work itself** — e.g. a mutating source edit that member's own rules require to go through a co-working partner or explicit human-owner confirmation before it can land. Name that partner directly on this line, or state that human-owner confirmation already happened — don't leave the receiving member to notice the gate and reconstruct it from its own file on every occurrence. Write `execution-gate: none` only once the receiving member's own `.armed.md` has actually been checked for one, never as a default when it wasn't looked at.
   - `tool-routing:` — **the execution mechanism this dispatch's own shell/tooling calls must use** — `mcp__myx_distro__execute`, never Bash or any other direct-execution path, for any dispatch touching this team's own tooling; the specific correct mechanism instead for a dispatch in a different context that doesn't use this team's tooling at all. Write `tool-routing: mcp__myx_distro__execute only, never Bash/direct execution` as that standard value, or the mechanism that actually applies — never a bare omission.
...
- Eligible: an item that passed through `board-processed` before landing here — signaled by `processed-at`, which is stamped by every operation that puts a document into a `processed/` folder and by nothing else, and is never re-stamped or cleared by any later move. Those operations are `--magic-grooming-to-processed`, `--magic-grooming-create-processed`, `--magic-board-to-processed` (which skips the stamp on a same-state `processed`→`processed` edit), `--intern-op-board-rename`'s own husk into `board/processed/`, and `--intern-op-inbox-to-processed` for the per-member log-entry population (whose items are not board-items and never reach `board-archived`, so they do not enter this predicate at all). The marker is the *fact of having been in `board-processed`*, not one routine's authorship of it — which is what keeps this predicate true as that list grows.
...
- `groomed-from` is not used as the marker: `--magic-grooming-to-archived` unconditionally re-stamps `groomed-from` to the caller's `--from-state:` on every call, including a same-state header-only edit — e.g. removing `archive: true` via `--from-state:archived --header:remove:archive` silently overwrites `groomed-from: processed` to `groomed-from: archived` in the same motion. `processed-at` carries no such hazard and is used instead.
...
8. **board-reassess-archived-missing-flag**: Reassess `board-archived` items missing `archive: true`, narrowly scoped to the population that flag actually governs.
   ...
   - No signal this pass (no `board-archived` item currently carries `processed-at`): do nothing — this is the expected, correct outcome for a `board-archived` population that reached here via the direct-Drop path, and activates automatically once the `archive: true` GC diversion itself starts moving items here from `board-processed`.
...
- Verbatim-relay discipline is a workflow necessity for this role, not just hygiene — team authority means a receiving session is structurally inclined to defer to it, so a blended annotation risks being obeyed as the command itself. Per `magic-team/magic-team.conversations.md`'s **labeled-annotation-not-rephrasing**: label added remarks explicitly (e.g. `Consider this comment from relay party:`), never share a paragraph with the quote. On conflict, the relay always wins.
...
- **Carve-out, classifier denials only**: a tool call denied by the harness's own auto-mode permission classifier gets exactly one plain, unreworded retry before it counts as the rule above's stop-and-ask signal. Rewording the request to dodge the classifier stays forbidden — only a bare retry is added. Reason: this denial class is transient — it succeeds on a plain retry with no settings change and no request change, unlike a genuinely broken mechanism. Every other failure class still stops and asks after one failure, unchanged.
...
- An inquiry/request is "obvious" — resolve it inline, straight to done, no `inquiry-*` item needed — only when both hold: (a) no subtasks need decomposing, and (b) no assignee-transfer/hand-off is needed. If either fails, it's non-obvious: create/track it as a real `inquiry-*` item through the full lifecycle (`magic-team/magic-team.board.md`'s "General item lifecycle"). Filename: type prefix first, date immediately after, no extra words in between — `inquiry-<date>-<matter>.md`.
...
- **What to hand off**: compile and curate a dispatch's goal, rules, context, and inputs specifically for that task — never the coordinator's own sprawling, multi-topic session forwarded wholesale, and never an open-ended "check X, Y, or wherever" pointer that offloads the coordinator's own compilation work onto the spawned session. Binds a dispatch's initial goal and any later message sent into an already-running session alike — not just the first message:
  ...
  - An annotation added around a dispatch exists to help the receiving worker succeed, not to introduce avoidable failure of its own: verify a fact before presenting it as already checked/confirmed — an unverified claim dressed as settled fact is worse than no claim at all, this being the dispatch-annotation case of the report-is-evidence rule above rather than the whole of it; flag a genuine change to prior instructions as a change, never let it stand silently alongside the earlier guidance as if both remain equally valid; and keep the annotation's own framing plain and neutral rather than urgent or pressuring, since urgent/pressuring framing can trigger a safety refusal by itself, regardless of whether the underlying task is legitimate.
  - A bug/mechanism a dispatch brief describes is read in full before it's written down, not summarized from the first plausible-looking part of a partial read — the dispatch-brief-specific case of the read-the-whole-mechanism discipline `magic-team/magic-team.armed.md`'s Engineering & operating discipline section states generally. A brief compiled from an incomplete read hands the receiving worker a wrong initial diagnosis to chase, which then has to be found and walked back once that worker reads the whole mechanism itself — the curation this section already calls for only holds up if what's being curated was actually read completely first.
...
**Reload trigger for the orchestration layer itself**: every 20 loop iterations, or immediately once any of this session's own loaded skill files (`magic-coordinator.harness.md`, `magic-coordinator.armed.md`, `magic-team/magic-team.armed.md`, and any `.routine.md` file this iteration is about to execute) shows a newer mtime than when this session last loaded it, whichever comes first, re-run `Skill(magic-coordinator)` fresh before continuing the cycle. Track both the iteration counter and each watched file's last-loaded mtime as session-local state, reset both the moment a reload completes.

**Same reload mechanism, generalized to `coordination-session`.** `coordination-session` already runs a repeating cycle of the same shape as `main-loop-mode`: **sweep-comms** → **advance-on-update** → **drive-session-goal** → **narrow-goal-gap** → **pause-between-cycles** → **repeat-from-sweep**, above. It gets the identical reload trigger: every 20 completed cycles, or immediately once any of the same watched files (`magic-coordinator.harness.md`, `magic-coordinator.armed.md`, `magic-team/magic-team.armed.md`, or any `.routine.md` about to execute) shows a newer mtime than last loaded, whichever comes first. Reload happens at the top of the next **sweep-comms** step, never mid-step.

This is the same mechanism, not a variant. The reload itself is a fresh `Skill(magic-coordinator)` re-invocation, never a partial re-read of individual files — a partial re-read would leave a stale mix of refreshed and carried-over sections. `coordination-session` tracks the iteration counter plus each watched file's last-loaded mtime as session-local state, reset on reload completion — identical bookkeeping to `main-loop-mode`'s own.

This is what keeps a long-running `coordination-session`'s adherence to the harness-session doctrine from drifting as its own conversation grows and the original skill-file instructions fall further back in context.

**This does not generalize to plain one-shot `armed-mode`.** It has no cycle to count against. A one-shot session running long enough to need drift-correction should have been split into a fresh session instead of given its own reload cadence — see `magic-team/magic-team.armed.md`'s "one spawned session holds one line of work" rule and this file's own "reuse an already-open member session" rule above.
...
- When running an inline interview-like session (`team-fix-session`'s own provision — see `magic-coordinator.harness.md`'s "Interview-like sessions, inline"), this session follows `magic-team.interview.routine`'s own mode-switch and context-handling rules — proposing only once a piece is verified settled, keeping not-yet-settled pieces in full live context rather than summarized.

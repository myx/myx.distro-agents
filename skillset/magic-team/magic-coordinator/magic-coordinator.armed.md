...
**Same reload mechanism, generalized to `coordination-session`**: `coordination-session` already runs a repeating cycle of the same loop-shape (**sweep-comms** → **advance-on-update** → **drive-session-goal** → **narrow-goal-gap** → **pause-between-cycles** → **repeat-from-sweep**, above), so it carries the identical reload trigger unchanged — every 20 completed cycles of its own loop, or immediately once any of the same watched files shows a newer mtime than last loaded, whichever comes first; reload at the top of the next **sweep-comms** step, never mid-step. Same mechanism, not a variant: re-invoke `Skill(magic-coordinator)` fresh (never a partial re-read of individual files, which would leave a stale mix of refreshed and carried-over sections), and track the iteration counter plus each watched file's last-loaded mtime as session-local state, reset on reload completion — identical bookkeeping to `main-loop-mode`'s own. This is what keeps a long-running `coordination-session`'s adherence to the harness-session doctrine from drifting as its own conversation grows and the original skill-file instructions fall further back in context. **This does not generalize to plain one-shot `armed-mode`**: it has no cycle to count against, and a one-shot session running long enough to need drift-correction should have been split into a fresh session instead of given its own reload cadence — see `magic-team/magic-team.armed.md`'s "one spawned session holds one line of work" rule and this file's own "reuse an already-open member session" rule above.

...
- Verbatim-relay discipline is a workflow necessity for this role, not just hygiene — team authority means a receiving session is structurally inclined to defer to it, so a blended annotation risks being obeyed as the command itself. Per `magic-team.conversations.md`'s **labeled-annotation-not-rephrasing**: label added remarks explicitly (e.g. `Consider this comment from relay party:`), never share a paragraph with the quote. On conflict, the relay always wins.
...
- **What to hand off**: compile and curate a dispatch's goal, rules, context, and inputs specifically for that task — never the coordinator's own sprawling, multi-topic session forwarded wholesale, and never an open-ended "check X, Y, or wherever" pointer that offloads the coordinator's own compilation work onto the spawned session. Binds a dispatch's initial goal and any later message sent into an already-running session alike — not just the first message:
  ...
  - A bug/mechanism a dispatch brief describes is read in full before it's written down, not summarized from the first plausible-looking part of a partial read — the dispatch-brief-specific case of the read-the-whole-mechanism discipline `magic-team.armed.md`'s Engineering & operating discipline section states generally. A brief compiled from an incomplete read hands the receiving worker a wrong initial diagnosis to chase, which then has to be found and walked back once that worker reads the whole mechanism itself — the curation this section already calls for only holds up if what's being curated was actually read completely first.
...
- `magic-team.authority.keeper.contract.md` — decision-authority and relationship-shape contract for `keeper-*`, cross-referenced from all keepers' own definitions.
- `magic-team.authority.warden.contract.md` — same contract shape for `warden-*`, keeper's pairing counterpart; no real instance yet.
- `magic-team.authority.partner.contract.md` — same contract shape for `partner-*`, cross-referenced from `partner-ndm-camunda`/`partner-ndm-infra`/`partner-ndm-ndxs`.
- `magic-team.authority.client.contract.md` — same contract shape for `client-*`, cross-referenced from `client-ndm`.
- `magic-team.authority.oncall.contract.md` / `magic-team.authority.expert.contract.md` — same contract shape for `oncall-*`/`expert-*`; no real instance of either yet.
...
- Every `keeper-*`/`warden-*` member — domain grounding; see `magic-team.authority.keeper.contract.md`/`magic-team.authority.warden.contract.md`.
- Every `partner-*`/`client-*` member — external-relationship grounding; see `magic-team.authority.partner.contract.md`/`magic-team.authority.client.contract.md`.
- Any `oncall-*`/`expert-*` engagement, once one exists — see `magic-team.authority.oncall.contract.md`/`magic-team.authority.expert.contract.md`.

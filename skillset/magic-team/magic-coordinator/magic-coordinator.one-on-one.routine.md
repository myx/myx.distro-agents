...
2. **Prep context**: pull any relevant board items owned by or referencing this member (including `board-processed` `note-member-status-*` for pre-2026-07-22 history, since `TEAM-STATUS.md` itself is now a retired stub), and any relevant project memory so the handoff isn't a cold start.

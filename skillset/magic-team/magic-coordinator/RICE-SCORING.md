...
Per the team's own "no unilateral epics" guardrail (`magic-coordinator.heartbeat.routine.md`, `magic-coordinator.communication-sweep.routine.md`) and `routine-grooming`'s own **review-with-the-user**: the reprioritized, scored backlog is still reviewed *with the user* before anything is considered final. A score is an input to that conversation, not a substitute for it.
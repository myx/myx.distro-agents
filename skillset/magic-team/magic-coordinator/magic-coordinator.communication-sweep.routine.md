...
- Two source sets, both read every sweep, in this same pass: the executor's own team-scoped sources, and every `client-*` member's own external sources — each read under that member's own credentials, never under the team's. A `client-*` member is an avatar of this routine's own executor, which is why its traffic is swept here; no routine belongs to one particular client, and none is written.
- **`client-*` member set**: the `client-*` skill directories under the skillset root, listed as one glob at the start of **check** — `ls -d "$HOME"/.claude/skills/client-*`. That directory is what the scan operation itself resolves a member against, so the two cannot disagree, and a member added later is swept with no edit to this file. No operation enumerates members: the roster record is a hand-maintained cache and drifts from what is on disk, and no listing operation exists. An enumeration operation is a real tooling gap; until it is closed, the glob is the instrument.
...
2. **check**: read the sweep-state-note via --magic-sweep-state-read. Call --magic-sweep-input-scan <team-member> --comms-since-utime <last_swept_ts> — board-tracked threads and every watched source, one pass, covering both watched targets and every open thread (see Scope).
   ...
   - every `client-*` member is scanned in this same pass, one `--client-sweep-input-scan <client-* member> --comms-since-utime 0` call each. The cut-off is passed on every call, never left to the operation to supply. One member's call failing ends that member alone: the remaining members are still called, and no member's result is ever recorded from another's.
   - a `client-*` member's own document is one ordered run of its own, appended after the combined set above rather than merged into it: that document orders messages within a source and never across them, so the single ascending set covers the team's own sources only. Inside it each section is read in the order it declares for itself — the chat section states its own `sort:` line, the mail section lists ascending UID and so is already oldest first, and the Trello section declares none and none is established elsewhere, so no order is assumed for it.
   - that same document carries the member's own inbox and board items. They are read to establish what is already recorded, never answered outward into that member's systems — a member's own inbox is `magic-team.process-inbox.routine`'s to work. Inbox blocks are consumed by their own `body-lines: <N>` count, never by scanning forward to the next `## ` heading: an inbox body legally contains `## ` lines, and a heading scan mis-slices the document.
...
3. **process-each-message**: for every message in **check**'s own ascending-time-ordered set, in that order, then each `client-*` member's own run in its own declared order, one message at a time — not as five separate batch passes over the whole set. A message from a `client-*` member's own source runs this same sequence under that member's own identity: that member is the `<team-member>` argument of every call the message needs, and the credentials used are its own. Each message runs this full local sequence before moving to the next:
...
1. **update-context**: fold platform mechanical-state findings into the `sweep-state-note` via `--magic-sweep-state-upsert` — `--edit-patch-from-stdin` for a single-field update, full-content write only for a genuine whole-record rewrite — invoked through `mcp__myx_distro__execute` only. Fold identity/routing data into the `roster-note` via `--member-inbox-note-upsert`.
   ...
   - **On a `client-*` source, idempotence is the per-message marker, not `last_swept_ts`.** That record is this routine's own single high-water mark and holds no per-member position, which is why each `client-*` scan passes `0` and re-reads that member's whole history every pass. What stops a message being worked twice is the mark **reply-if-warranted** leaves on it under that member's own identity — the chat reaction ladder, and `--member-comms-email-mark-seen` for mail. Mail is the case that fails silently without it: the scan reads message bodies without setting the seen flag, so an unmarked mailbox re-reports the same messages as new on every later pass.
...
- `--client-sweep-input-scan <client-* member> [--comms-since-utime <v>|--comms-since-date-time <v>]` (**check**: one `client-*` member's own external sources, read under that member's own credentials)
...
## `--client-sweep-input-scan` Operation Reference

`DistroAgentsTools.fn.sh --client-sweep-input-scan <client-* member> [--comms-since-utime <v>|--comms-since-date-time <v>]` — reads one `client-*` member's own incoming external communications, under that member's own credentials only, plus that member's own board items and inbox, as one document. Runs only as a `client-*` member; any other member is refused, naming what was wrong. The member name and the optional cut-off are its only arguments — an item name is not a parameter to it. The two cut-off spellings are mutually exclusive and neither is repeatable; `--comms-since-utime` takes epoch seconds, `--comms-since-date-time` a `YYYY-MM-DD`-leading value. Exit status: `0` every source scanned, `3` some scanned and some not, `4` none, `1` the operation failed — a failed call may already have written part of a document to stdout, so the status is what is read, never the presence of output. An optional source the member holds no credentials of its own for is counted in no source total and does not make the scan partial.
...
- A second `client-*` member joins the team. The next sweep reads it under its own credentials, found by the member glob, with no edit to this file and no file anywhere naming it.
- One `client-*` member's scan call fails. The remaining members are still scanned, and none of them is recorded as clean on the strength of the failed one.
- A `client-*` member's mail is read and answered but never marked seen. The next sweep reports the same messages as new — the per-message marker, not the stored high-water mark, is what makes that member's pass idempotent.

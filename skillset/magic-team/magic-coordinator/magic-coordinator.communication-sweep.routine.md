...
`DistroAgentsTools.fn.sh --sweep-read-incoming-comms [--oldest <ts>] [--raw]` — not a general-purpose Slack reader, takes no target at all. The dedicated macro-operation for this routine's own **check** step: reads the exact predefined watched-source set (both Slack targets via `--check-slack`, plus `--check-email` and `--check-trello`) in one combined pass. For Slack, the watched-channel history pass also auto-follows any thread parent visible in that returned page whose `reply_count` is non-zero and whose `latest_reply` is newer than `--oldest`, by issuing the same `--check-slack <channel>:<ts>` call internally — widening coverage to freshly-active watched-channel threads not yet tracked on the board. It separately also follows a thread — still only within the two watched channels' own returned history page — whose parent was posted by Vane, already has Vane among its participants, or tags Vane in its own text, bypassing the freshness gate for that thread. Vane's own identity is resolved once per sweep through the team's existing identity-resolution mechanism; if that resolution fails, this widening is skipped for that pass and only the base freshness heuristic applies. This is still not a workspace-wide mention search: older untracked thread parents outside that history page, or a Vane mention/participation somewhere outside the already-watched sources, remain undiscoverable here — true "tagged anywhere" coverage is a separate, not-yet-built capability. For one specific arbitrary Slack target/thread, call `--check-slack` directly instead.

...
- Does:
  ...
  - Comms write-half.
  - Mechanical board moves, same as `routine-advance` (**run-check-process-board**).
...
- Doesn't do:
  - Spawn a new dispatch — never calls `check-execute-board` (`magic-coordinator.advance.routine.md`, `routine-advance`-only: starts a never-yet-dispatched `board-pending` item, restarts/nudges `board-running` work). **fan-out-work-sessions**' own member fan-out is the one exception.
...
1. **acquire-lock**: Acquire this routine's own lock — a single `--magic-daily-lock-acquire` call, before anything else in this routine runs, **spawn-morning-review**'s spawn included. `ACQUIRED`, or a reclaim of a dead holder's lock, means go. Contention means another `routine-daily` is live: this pass does not start, and nothing below runs.
2. **spawn-morning-review**: first-today only — spawn `routine-librarian-morning-review` as a full sub-session, and wait for it to complete.
   ...
   - Skip silently on a same-day re-run of `routine-daily` — once per workday, same first-today gating as **run-check-process-board**'s grooming counterpart.
...
6. **sort-incoming**, in parallel with **librarian-confirms-roster** and **sweep-comms-read**: run `routine-process-inbox magic-coordinator` — inline execution (own identity). A genuine second pass, not a repeat of **collect-reflections-output**: that one picks up what **fold-in-learned-lessons** just filed, this one is the sorting read across everyone's `inquiry-*`/`reflection-*`/status reports and reactions accumulated since the last daily, so **roll-call** speaks to what actually came in.
   - Not automatic just because this routine spawned — this explicit call is what actually guarantees it happens.
7. **sweep-comms-read**, in parallel with **librarian-confirms-roster**: run the read half of `routine-communication-sweep` — not Trello-specific, covers every live platform under `the credential store`.
   - Fold anything relevant into the roll call narration in **roll-call**.
   - Don't act on it unilaterally.
8. **ping-stale-interviews**: check active human-owner interviews, ping for reply — for every open `routine-interview` thread `magic-coordinator` is itself the live counterpart in, that's awaiting a human-owner reply past a staleness window, send one of two message patterns, never both, via the `--member-comms-slack-send-message` operation:
   ...
   - Ground "what's changed" in this session's own **roll-call** and **sweep-comms-read** findings, not a fresh investigation.
...
9. **roll-call**: not a status meeting -- narrating status without adapting the plan is the failure mode this step must avoid; **update-todos** immediately after must actually change the plan, not just record what was said. random order — pick a random order over the permanent members (every skill except `magic-coordinator` itself).
   - Fixed question order: what happened since last time, what's planned next, what's blocking -- in that order, for each member. Call the `--member-work-session-input-scan` operation, then narrate that member's status/blocks/ideas/leftovers for today from it plus whatever's visible in `TodoWrite`/project memory.
...
10. **update-todos**: reflect what the roll call surfaced in the current session's `TodoWrite` (today's working list) for the members about to get a work session.
...
11. **run-check-process-board**: run the `check-process-board` procedure (`magic-coordinator.armed.md`) directly. Never `routine-advance`.
12. **librarian-updates-context**: today's new task details already live on the board directly (via **run-check-process-board**'s pass and **fan-out-work-sessions**' dispatched agents' own board moves) — no separate write-back step exists.
13. **sync-camunda-diagrams**: run `routine-camunda-diagram-sync` — mtime check on the `temp-magic-team` BPMN diagrams against team definition files, redeploy handoff to that routine's own owning `partner-*` if stale.
...
14. **questions-then-conclude**: let the user (or a member, narrated) ask anything before closing the standup portion.
...
15. **fan-out-work-sessions**: for every permanent member that has something to do today — **except `partner-*`** (present-but-non-reporting members don't get a work session either) — spawn one background `Agent` per member.
...
16. **sweep-comms-write**: run the write half of `routine-communication-sweep` — update the own-status card to reflect today's actual state, and reply/comment anywhere else warranted across whatever platforms are live.
...
1. **run-advance-dispatch**: run `routine-advance`, in full, before anything else here — it is the routine that actually dispatches and respawns; the main sequence deliberately doesn't.
   ...
   - rule: this does not re-dispatch **fan-out-work-sessions**' own work sessions — `check-execute-board`'s standing "`routine-daily`'s standing work-sessions take continuous task feed as each finishes" exception already excludes them.
   - rule: `routine-advance`'s **advance-run-process-board** repeats the `check-process-board` pass **run-check-process-board** already ran. That is a deliberate second reconciliation over a board the main sequence has since changed, not an accident — `check-process-board` executes only already-decided moves, so re-running it is safe.
...
- **Executor vs. participant role, defined explicitly** (resolving this routine's own version of the "always (sub-)spawned" standing rule): `routine-daily` has one **executor** (`magic-coordinator`) and, at its work-session fan-out step (**fan-out-work-sessions**), several **participants** — every member that step actually dispatches. This is `routine-daily`'s own version of the same executor/participant distinction `routine-coworking` already draws explicitly ("other members are participants, not executors"), just shaped around this routine's own "fan-out" collaboration mode (each participant works its *own* separate assignment) rather than several people on one shared task.
...
- `check-process-board` (`magic-coordinator.armed.md`) — dependency-ordering recompute, **run-check-process-board**; called directly, not via `routine-advance`.
...
- `routine-camunda-diagram-sync` — BPMN diagram staleness check, **sync-camunda-diagrams**.
- `routine-communication-sweep` — read half (**sweep-comms-read**) and write half (**sweep-comms-write**).
- `routine-process-inbox` — the sorting pass over own inbox, **sort-incoming**; see its own "Execution mode is decided by identity match" section for why this explicit call is required.

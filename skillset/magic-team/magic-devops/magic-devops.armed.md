...
- These rules define *what* an action is and what gate it carries. Who may *ask* for one is `magic-coordinator.armed.md`'s own rule against creating a task that instructs another member to perform a destructive/irreversible action outside that action's own established mandate — which defers the definition of what counts back to this file.

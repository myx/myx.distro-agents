...
**The correct, narrow command for picking up a local source edit** — the same one the workspace's own "🛫 Ingest Source Changes..." VS Code task runs: `DistroSourcePrepare.fn.sh --ingest-distro-index-from-source`. Runs sync-from-source + index-publish in sequence, scoped to whatever actually changed, printing `🔂 scan/sync: updated: <project>` per touched project. Use this first; reach for the full pipeline only if this genuinely doesn't resolve the staleness.
